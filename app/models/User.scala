...
object User {
  ...
  def findByEmail(email: String): Future[Option[User]] = Future.successful {
    ...
    try {
      ...
      val rs = stmt.executeQuery("SELECT * from users where username='" + email + "'")
      if (rs.next()) {
        Option[User](User(1L, email, rs.getString("password"), "User Name", true, true))
      } else Option[User](null)
    } finally {
      conn.close()
    }
  }
