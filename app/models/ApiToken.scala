...
object ApiToken {
  ...
  def create(apiKey: String, userId: Long, username: String): Future[String] = Future.successful {
    ...
    val token = newUUID
    tokens.insert(_ => ApiToken(token, apiKey, expirationTime = (new DateTime()) plusMinutes 10, userId, username))
    token
  }
